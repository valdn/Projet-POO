...
sf::Vector2f Rectangle::getPoint(std::size_t index) const {
	...
	switch (index) {
	default:
	case 0: return sf::Vector2f(x, y);	//Premier point ancre coin sup�rieur gauche
	case 1: return sf::Vector2f(largeurf + x, y);	//coin sup�rieur droit
	case 2: return sf::Vector2f(largeurf + x, y + hauteur);	//coin inf�rieur droit
	case 3: return sf::Vector2f(x, y + hauteurf);	//coin inf�rieur gauche
	}
}
