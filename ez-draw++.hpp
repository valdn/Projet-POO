 ...
 *
 * eric.remy@univ-amu.fr - 01/03/2014 - version 1.2
 *
...
/// \note Attention la modification de la largeur ainsi produite n'est qu'une demande qui s'ajoute à la file d'attente, et qui ne sera pas effective tant que la boucle de traitement d'événement ne l'aura pas prise en compte (cf. « EZDraw::mainLoop() »). De ce fait, si vous souhaitez modifier la largeur ET la hauteur simultanément, il est nécessaire d'utiliser « EZWindow::setWidthHeight(unsigned int w, unsigned int h) ».
...
/// Convertit un triplet de valeurs de rouge, vert et bleu en un identifiant de couleur.
/// \param red,green,blue composantes rouge, vertes et bleues, entre 0 et 255.
/// \return un entier long correspondant à la couleur demandée mais dans une représentation spécifique au système d'exploitation utilisé.
 static EZColor          getRGB       (EZuint8 red, EZuint8 green, EZuint8 blue);

/// Convertit un triplet de valeurs teinte, saturation et valeur (voir [Wikipédia](https://fr.wikipedia.org/wiki/Teinte_Saturation_Valeur)) en un identifiant de couleur.
/// \param hue, saturation, value Les valeurs (respectivement) de teinte, saturation et valeur.
 static EZColor          getHSV       (double hue, double saturation, double value);

/// Convertit une valeur de gris en un identifiant de couleur.
/// \param gray niveau de gris entre 0 et 255.
/// \return un entier long correspondant à la couleur demandée mais dans une représentation spécifique au système d'exploitation utilisé.
 static EZColor          getGrey      (unsigned long gray);

/// Convertit une valeur de teinte, saturation et valeur en un triplet rouge, vert et bleu.
/// \param hue,saturation,value Les valeurs de teinte, saturation et valeur.
/// \param red, green, blue Les variables, passées par référence, dans lesquelles seront stockées les valeurs résultantes rouge, vert et bleu.
...
/// Utiliser startTimer() pour le relancer avec un nouveau délai.
 ...
 /// Cette fonction virtuelle est appelée à chaque mouvent de la souris dans la fenêtre.
...
public:
 ...
 /// Les formats supportés sont : PNG, GIF, BMP, JPEG. Les formats PNG et PMP supportent des degrés variables de transparence. Le format GIF ne permet que de la transparence en tout ou rien. Le format JPEG ne permet pas de représenter la transparence.
